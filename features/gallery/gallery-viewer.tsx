        ...
        {caseStudiesData.map((caseStudy) => (
          <GlassCard key={caseStudy.id} variant="standard" className="space-y-4 group bg-white border border-slate-200 shadow-md hover:shadow-xl hover:border-medical-500/40 transition-all duration-400">
            <div
              ...
              className="grid grid-cols-2 gap-2 rounded-xl overflow-hidden border border-slate-200 cursor-pointer relative touch-manipulation select-none active:scale-[0.99] transition-transform"
            >
              <div className="relative overflow-hidden">
                <img
                  src={caseStudy.beforeImage}
                  alt="Before"
                  className="w-full h-40 sm:h-48 object-cover group-hover:scale-105 transition-transform duration-500 ease-out"
                />
              ...
              <div className="absolute inset-0 bg-navy-900/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                <span className="inline-flex items-center px-4 py-2 rounded-full bg-white text-xs font-bold text-navy-900 shadow-xl group-hover:scale-105 transition-transform pointer-events-none">
                  <Eye className="mr-1.5 h-4 w-4 text-medical-600" /> Tap to Inspect Case
                </span>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-slate-500">
              ...
              <Button
                ...
                size="sm"
                onClick={() => handleOpenCase(caseStudy)}
                className="w-full sm:w-auto font-bold text-xs h-10 touch-manipulation cursor-pointer justify-center"
              >
